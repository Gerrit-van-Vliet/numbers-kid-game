...
// userName now comes from centralized settings
const messages = {
    click: {
        en: 'Click on: ',
        nl: 'Klik op : '
...
function motivateToClickSpecificNumber(number) {
    if (ttsEnabled.value) {
        speak(messages.click[language.value] + String(number), { interrupt: true })
    }
}

function onApplySettings(settings) {
    if (typeof settings?.hapticsOn === 'boolean') {
        hapticsEnabled.value = settings.hapticsOn
    }
    if (settings?.language) {
        setLanguage(settings.language)
    }
    if (typeof settings?.ttsOn === 'boolean') {
        ttsEnabled.value = settings.ttsOn
    }
    if (settings?.ttsVoiceUri) {
        selectVoiceByUri(settings.ttsVoiceUri)
    }
}
